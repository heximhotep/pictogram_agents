...
def get_model_and_assets():
  ...
  return common.read_model('wobble.xml'), common.ASSETS
...
class Physics(mujoco.Physics):
  ...
  def mouth_to_target(self):
    ...
    return mouth_to_target_global.dot(data.geom_xmat['head'].reshape(3, 3))
...
class Swim(base.Task):
  ...
  def get_reward(self, physics):
    """Returns a smooth reward."""
    radii = physics.named.model.geom_size[['head', 'target'], 0].sum()
